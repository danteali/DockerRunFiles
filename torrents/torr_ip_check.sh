...
if [ ! -z "$(docker ps | grep torr_jackett | grep Up)" ]; then
  ...
  RADIP=`cat $SOURCE/jackett_ip.tmp | grep -e ip | tr -d '\n\r ,"ip:'`
  RADCO=`cat $SOURCE/jackett_ip.tmp | grep -e country | tr -d '", ' | sed 's/country://'`
  rm $SOURCE/jackett_ip.tmp
  echo "Radarr IP Address: ${green}$JACKIP${reset}"
  echo "Radarr Country: ${green}$JACKCO${reset}"
  echo "" && echo "--------------------------------------" && echo
else
  echo "${red}Radarr container not running${reset}"
  echo "" && echo "--------------------------------------" && echo
